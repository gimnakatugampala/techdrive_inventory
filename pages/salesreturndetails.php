...
// Get the Sales Order Return ID
if ( $result->num_rows > 0 ) {
    $id = $result->fetch_assoc();
}

// Get the Customer & Sales Return Invoice
$sqlsalesreturn = "SELECT *,tbsalesreturninvoice.discount AS DIS FROM tbsalesorderreturn 
...
$resultsales = $conn->query( $sqlsalesreturn );

$salesordersreturn = array();
if ( $resultsales->num_rows > 0 ) {
    while ( $row = $resultsales->fetch_assoc() ) {
        $salesordersreturn[] = $row;
    }
}


// echo json_encode( $salesordersreturn );
