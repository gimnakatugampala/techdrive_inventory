...
}else if($TYPE == "SO_COMPLETED"){

	$inv_mst_query = "SELECT * FROM tbsalesorder WHERE socode ='".$MST_ID."' AND sid=1 ";             
	$inv_mst_results = mysqli_query($conn,$inv_mst_query);   
	$count = mysqli_num_rows($inv_mst_results);  
...
}else if($TYPE == "SO_CANCELED"){

	$inv_mst_query = "SELECT * FROM tbsalesorder WHERE socode ='".$MST_ID."' AND sid=3";             
	$inv_mst_results = mysqli_query($conn,$inv_mst_query);   
	$count = mysqli_num_rows($inv_mst_results);  
...
// echo "Upload successfully!!";
	...
	if($TYPE == "SO_INPROGRESS"){
		...
		// SEND INVOICE IN THE EMAIL
		$email_invoice_path=$file_location.$file_name;$EMAIL_STATS="COMPLETED"; require_once '../utils/send_email.php';
	}else if($TYPE == "SO_CANCELED"){
		// SEND INVOICE IN THE EMAIL
		$email_invoice_path=$file_location.$file_name;$EMAIL_STATS="CANCELED"; require_once '../utils/send_email.php';
	}else if($TYPE == "QUO"){
